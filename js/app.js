...
function renderSemGrid(container) {
  ...
  for (let i = 1; i <= CONFIG.semesters; i++) {
    ...
    card.innerHTML = `
      ...
      <div class="sem-card-sub">5 Subjects</div>
    `;
...
function renderSubjectGrid(container, sem) {
  if (!container) return;
  container.innerHTML = '';
  const classes = ['accent-physics','accent-chemistry','accent-botany','accent-zoology','accent-maths'];
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (page === 'semester') {
    ...
    if (sub) sub.textContent = `Choose a subject to explore notes`;
